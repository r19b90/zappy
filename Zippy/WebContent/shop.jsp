...
<body>
<section>
<%@ page import="com.myapp.MyDao.MyDao,com.myapp.bean.CustomerBean,java.util.ArrayList" %>
<% MyDao m = new MyDao();
		ArrayList<CustomerBean> list = m.viewProduct();%>
	...
	<table border = 5>
		...
		<th>	<td> <%String iname= e.getImage(); %>
 				<img src="images\<%=iname%>" heigth="200" width="200"/> </td></th>
 				
 			<th><td><%= e.getProductID()%> <input type="hidden" value="<%= e.getProductID()%>" name="pid" /><br/>
			<%= e.getName()%> <input type="hidden" value="<%= e.getName()%>" name="pname" /><br/>
			<%= e.getPrice()%><input type="hidden" value="<%= e.getPrice()%>" name="price" /> <br/>
			<%= e.getQuantity()%><input type="hidden" value="<%= e.getQuantity()%>" name="qty" /> <br/>
			<%= e.getDescription()%><input type="hidden" value="<%= e.getDescription()%>" name="desc" /><br/>
			<input type= "number" name="quanty"/><input type="submit" value="Add To Cart"></td></th>
			</form>
	</form>
	</tr>	
		<% }%>  
